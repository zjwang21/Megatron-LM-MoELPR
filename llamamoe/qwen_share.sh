...
if [ -n "${WANDB_API_KEY}" ]; then
    LOGGING_ARGS+=(
        ...
        --wandb-exp-name ${WANDB_NAME:-"qwen2.5_3b_instruct_tpsplit_12exp_top4_tp1_ep4"}
    )
fi

if [ ${NODE_RANK} -eq 7 ];then
torchrun ${DISTRIBUTED_ARGS[@]} ${MEGATRONN_PATH}/pretrain_gpt.py \
    ...
    ${TRAINING_ARGS[@]} \
    ${MODEL_PARALLEL_ARGS[@]} \
    ${LOGGING_ARGS[@]} > $WORK_DIR/Megatron-LM/llamamoe/elhutr30b_qwen2.5_3b_tpsplit_12exp_top4_share.out 2>&1
else
torchrun ${DISTRIBUTED_ARGS[@]} ${MEGATRONN_PATH}/pretrain_gpt.py \
    ${MODEL_ARGS[@]} \
    ${MOE_ARGS[@]} \
