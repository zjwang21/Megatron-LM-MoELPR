...
if [ -n "${WANDB_API_KEY}" ]; then
    LOGGING_ARGS+=(
        ...
        --wandb-exp-name ${WANDB_NAME:-"qwen2.5_3b_instruct_4exp_top2_tp1_ep2_review5w"}
    )
fi
...
torchrun ${DISTRIBUTED_ARGS[@]} ${MEGATRONN_PATH}/pretrain_gpt.py \
    ...
    ${TRAINING_ARGS[@]} \
    ${MODEL_PARALLEL_ARGS[@]} \
    ${LOGGING_ARGS[@]} > $WORK_DIR/Megatron-LM/llamamoe/elhutr30b_qwen2.5_3b_4exp_top2_sparse_review5w.out 2>&1
