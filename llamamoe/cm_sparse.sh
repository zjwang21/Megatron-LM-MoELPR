...
export NCCL_DEBUG_FILE=/root/work/huangxin/nanda/Megatron-LM/llamamoe/128_NCCL_DEBUG.out
export NCCL_TIMEOUT=1000000000
export NCCL_IB_GID_INDEX=3

# args
CHECKPOINT_PATH=/root/work/huangxin/nanda/models/cm-3b-mixtral-mcore-exp4-TP1PP1EP2
TOKENIZER_MODEL=/root/work/huangxin/ckpts/cm_3B
DATA_PATH=$WORK_DIR/data/data-bin/id_cm_mc4_culturax_text_document
LPR_LOSS_COEFF=1e-3
AUX_LOSS_COEFF=1e-3
LPR_STAGE=1
NUM_EXPERTS=4
...
torchrun ${DISTRIBUTED_ARGS[@]} ${MEGATRONN_PATH}/pretrain_gpt.py \
    ...
    ${TRAINING_ARGS[@]} \
    ${MODEL_PARALLEL_ARGS[@]} \
    ${LOGGING_ARGS[@]} > $WORK_DIR/Megatron-LM/llamamoe/id50b_cm_3b_4exp_top2_sparse_rest20B.out 2>&1
else
torchrun ${DISTRIBUTED_ARGS[@]} ${MEGATRONN_PATH}/pretrain_gpt.py \
    ${MODEL_ARGS[@]} \
    ${MOE_ARGS[@]} \
