...
def add_arguments(parser):
    ...
    group.add_argument('--init-experts', action="store_true",
                       help='Base directory of deepspeed repository')
...
def load_args_from_checkpoint(args):
    ...
    qwenmoe_config = Qwen2MoeConfig.from_pretrained(args.load)

    # Update Megatron args.
    args.untie_embeddings_and_output_weights = not qwenmoe_config.tie_word_embeddings
    ...
    args.max_position_embeddings = qwenmoe_config.max_position_embeddings
    args.hidden_size = qwenmoe_config.hidden_size
    args.num_attention_heads = qwenmoe_config.num_attention_heads
    args.num_layers = qwenmoe_config.num_hidden_layers
    args.norm_epsilon = qwenmoe_config.rms_norm_eps
    args.vocab_size = qwenmoe_config.vocab_size
    args.padded_vocab_size = qwenmoe_config.vocab_size
    args.qwenmoe = qwenmoe_config
    args.ffn_hidden_size = qwenmoe_config.intermediate_size
    args.moe_ffn_hidden_size = qwenmoe_config.moe_intermediate_size
    args.num_experts = qwenmoe_config.num_experts
    args.sequence_parallel = True

    if qwenmoe_config.num_key_value_heads:
        args.group_query_attention = True
        args.num_query_groups = qwenmoe_config.num_key_value_heads
...
def set_attn_state(args, layer, hf_layer):
    ...
    attn.linear_qkv.weight.data.copy_(torch.cat([
        hf_attn.q_proj.weight.reshape((num_query_groups, num_querys_per_group*dim, -1)), # 2 x 1024 x 2048
        hf_attn.k_proj.weight.reshape((num_query_groups, dim, -1)), # 2 x 128 x 2048
...
def set_mlp_state(args, layer, hf_layer, converter_args):
    '''Set MLP params.'''

    layer.mlp.router.weight.data.copy_(hf_layer.mlp.gate.weight)
    layer.mlp.shared_expert_gate.weight.data.copy_(hf_layer.mlp.shared_expert_gate.weight)

    layer.mlp.shared_expert.linear_fc1.weight.data.copy_(
        torch.cat([
    ...
    if not converter_args.init_experts:
        ...
        for expert_idx in range(args.num_experts):
            torch.nn.init.xavier_normal_(mcore_experts[expert_idx].linear_fc1.weight)
            torch.nn.init.xavier_normal_(mcore_experts[expert_idx].linear_fc2.weight)

def set_layer_state(args, model, hf_model, layer_idx, converter_args):
    ...
    set_mlp_state(args, layer, hf_layer, converter_args)
...
def load_checkpoint_to_model(args, converter_args):
    '''Set model params.'''
    ...
    hf_model = Qwen2MoeForCausalLM.from_pretrained(args.load, device_map="cpu")
    ...
    for layer_idx in tqdm(range(args.num_layers), "set layer states"):
        set_layer_state(args, model, hf_model, layer_idx, converter_args)
    return model
...
def _load_checkpoint(queue, args):
    ...
    model = load_checkpoint_to_model(margs,  args)
    ...
    for layer_idx in range(margs.num_layers):
        ...
        message["shared expert gate weight"] = layer.mlp.shared_expert_gate.weight.data
        ...
        if md.swiglu:
            ...
            chunked_shared_expert_l0_weight = torch.chunk(layer.mlp.shared_expert.linear_fc1.weight.data, 2, dim=0)
            message["shared expert W weight"] = chunked_shared_expert_l0_weight[0]
            message["shared expert V weight"] = chunked_shared_expert_l0_weight[1]
        else:
            message["mlp l0 weight"] = torch.stack([local_expert.linear_fc1.weight.data for local_expert in experts])

            message['shared expert l0 weight'] = layer.mlp.shared_expert.linear_fc1.weight.data

        message["mlp l1 weight"] = torch.stack([local_expert.linear_fc2.weight.data for local_expert in experts], dim=0)
        message['shared expert l1 weight'] = layer.mlp.shared_expert.linear_fc2.weight.data
