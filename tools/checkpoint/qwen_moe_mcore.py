...
def convert_checkpoint_from_megatron_to_transformers(mgmodel, hgmodel, args):
    ...
    with torch.no_grad():
        ...
        for mglayer, hglayer in zip(mgmodel.decoder.layers, hgmodel.model.layers):
            ...
            qkv_weight = mglayer.self_attention.linear_qkv.weight.view(num_query_groups, -1, head_dim, hidden_dim)
            ...
            shared_expert_gate_weight, shared_expert_up_weight = \
                torch.split(mglayer.mlp.shared_expert.linear_fc1.weight, split_size_or_sections=args.ffn_hidden_size)
...
def convert_checkpoint_from_transformers_to_megatron(hgmodel, mgmodel, args):
    ...
    with torch.no_grad():
        ...
        for mglayer, hglayer in zip(mgmodel.decoder.layers, hgmodel.model.layers):
            ...
            q_proj_weight = hglayer.self_attn.q_proj.weight.view(
                num_query_groups, -1, head_dim, hidden_dim
            )
            k_proj_weight = hglayer.self_attn.k_proj.weight.view(
                num_query_groups, -1, head_dim, hidden_dim
            )
            v_proj_weight = hglayer.self_attn.v_proj.weight.view(
                num_query_groups, -1, head_dim, hidden_dim
            )
            qkv_proj = (
            ...
            #k_bias = hglayer.self_attn.k_proj.bias.view([num_key_value_heads, -1])
            #v_bias = hglayer.self_attn.v_proj.bias.view([num_key_value_heads, -1])
...
def main():
    ...
    if args.convert_checkpoint_from_megatron_to_transformers:
        ...
        hf_model = AutoModelForCausalLM.from_pretrained(args.load, torch_dtype=torch.float16, trust_remote_code=True)
