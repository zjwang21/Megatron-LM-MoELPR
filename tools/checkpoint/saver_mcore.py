...
class MCoreMoETESetter(MCoreSetter):
    ...
    def set_layer(
        ...
        shared_expert_fc1_weight=None,
        shared_expert_fc2_weight=None,
    ):
        ...
        if shared_expert_gate_weight is not None:
            cls.set_tensor(l.mlp.shared_expert_gate.weight, shared_expert_gate_weight)
            cls.set_tensor(l.mlp.shared_expert.linear_fc1.weight, shared_expert_fc1_weight)
            cls.set_tensor(l.mlp.shared_expert.linear_fc2.weight, shared_expert_fc2_weight)
    ...
    for pp_rank in range(args.target_pipeline_parallel_size):
        ...
        for layer_id in range(len(setter.get_transformer_block(models[pp_rank][0][0]).layers)):
            ...
            if margs.num_experts:
                ...
                if margs.enable_shared_expert:
                    shared_expert_gate_weight = msg.pop("shared expert gate weight")
                    shared_expert_l1_weight = chunk_weight(msg.pop("shared expert l1 weight"), "row", args.target_tensor_parallel_size)
                    shared_expert_l0_weight_W = chunk_weight(msg.pop("shared expert W weight"), "column", args.target_tensor_parallel_size)
                    shared_expert_l0_weight_V = chunk_weight(msg.pop("shared expert V weight"), "column", args.target_tensor_parallel_size)
                    shared_expert_l0_weight = torch.cat((shared_expert_l0_weight_W, shared_expert_l0_weight_V), dim=-2)
                ...
                for tp_rank in range(args.target_tensor_parallel_size):
                    ...
                    if margs.num_experts:
                        ...
                        if margs.enable_shared_expert:
                            ...
                            params_dict.update({
                                "shared_expert_fc1_weight": shared_expert_l0_weight[tp_rank],
                                "shared_expert_fc2_weight": shared_expert_l1_weight[tp_rank],
                            })
